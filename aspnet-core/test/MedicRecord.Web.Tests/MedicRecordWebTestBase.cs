...
using MedicRecord.Models.TokenAuth;
...
public abstract class MedicRecordWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
{
    protected static readonly Lazy<string> ContentRootFolder;

    static MedicRecordWebTestBase()
    {
    ...
    protected override IWebHostBuilder CreateWebHostBuilder()
    {
        return base
            ...
            .UseSetting(WebHostDefaults.ApplicationKey, typeof(MedicRecordWebMvcModule).Assembly.FullName);
    }
    ...
    {
        using (var context = IocManager.Resolve<MedicRecordDbContext>())
        {
            action(context);
            context.SaveChanges();
        }
    ...
    protected async Task UsingDbContextAsync(Func<MedicRecordDbContext, Task> action)
    {
        using (var context = IocManager.Resolve<MedicRecordDbContext>())
        {
    ...
    protected async Task<T> UsingDbContextAsync<T>(Func<MedicRecordDbContext, Task<T>> func)
    {
        T result;
